...
const App: React.FC = () => {
  ...
  const handleAddProject = () => {
    ...
    if (name) setProjects(prev => [...prev, { id: Date.now().toString(), name }]);
  };
  ...
  const handleRenameTrigger = (id: string) => {
    setContextMenu(null);
  ...
  const getUserMenuStyles = () => {
    ...
    return { position: 'fixed' as const, left: '12px', bottom: (window.innerHeight - rect.top + 8) + 'px', zIndex: 9999 };
  };
  ...
  const getModelMenuStyles = () => {
    ...
    return { position: 'fixed' as const, left: left + 'px', bottom: (window.innerHeight - rect.top + 8) + 'px', zIndex: 9999, minWidth: menuWidth + 'px' };
  };
    ...
    <div key={id} className={cn("list-item session-item", editingSessionId === id && "is-editing")} onContextMenu={(e) => handleSidebarContextMenu(e, id)}>
      {editingSessionId === id ? (
        ...
        <>
          ...
          <button className="session-more-btn" onClick={(e) => { e.stopPropagation(); handleSidebarContextMenu(e, id); }} title="Options">
    ...
    <div className="container" onDrop={(e) => e.preventDefault()} onDragOver={(e) => e.preventDefault()}>
      <nav className="title-bar">
        <div className="title-bar-left">
          <button className="title-btn" title="Back"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg></button>
          <button className="title-btn" title="Forward"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg></button>
        </div>
          ...
          <div className="sidebar-content">
            <div className="sidebar-row collapse-container">
               ...
               <button className={`collapse-btn floating-trigger`} onClick={() => setIsSidebarCollapsed(true)} title="Collapse sidebar">
                 <div className="btn-icon-box"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><line x1="9" y1="3" x2="9" y2="21" /></svg></div>
               </button>
            </div>
            ...
            <div className="sidebar-group primary-actions" style={{ display: 'flex', flexDirection: 'column', gap: '2px', marginBottom: '12px' }}>
               ...
               <button className={cn("sidebar-btn terminal-btn", isTerminalOpen && "active")} onClick={() => setIsTerminalOpen(!isTerminalOpen)}>
                 ...
                 <span style={{ fontSize: '0.8125rem', fontWeight: 400, color: 'var(--white)', letterSpacing: '0.05em', fontFamily: 'var(--font-sidebar)', display: 'flex', alignItems: 'center', flex: 1, justifyContent: 'space-between', paddingRight: '12px' }}>
                   Terminal
                   <svg className="hover-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ color: 'var(--gray-600)', opacity: 0, transition: 'opacity 0.2s ease, transform 0.2s ease', transform: 'translate(-4px, 4px)' }}><line x1="5" y1="19" x2="19" y2="5" /><polyline points="12 5 19 5 19 12" /></svg>
                 </span>
               </button>
               <button className="sidebar-btn more-btn" onClick={() => {}}>
                 <div className="btn-icon-box"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="1" /><circle cx="19" cy="12" r="1" /><circle cx="5" cy="12" r="1" /></svg></div>
                  ...
                  <div className={cn("sidebar-list accordion-content", isProjectsExpanded ? "expanded" : "collapsed")}>
                    ...
                    {projects.map(p => (
                      <div key={p.id} className="sidebar-btn">
                        <div className="btn-icon-box"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg></div>
            ...
            <footer className="sidebar-footer">
              <button ref={userBtnRef} className="sidebar-btn user-profile-btn" onClick={() => setIsUserMenuOpen(!isUserMenuOpen)}>
                <div className="btn-icon-box"><div className="avatar">JD</div></div>
                <span>John Doe</span>
              </button>
            </footer>
          </div>
      ...
      {isUserMenuOpen && (
        <div className="popover-menu" style={getUserMenuStyles()} ref={userMenuRef}>
          <div className="menu-item" onClick={() => setIsUserMenuOpen(false)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Profile</div>
          <div className="menu-item" onClick={() => setIsUserMenuOpen(false)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1-2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>Settings</div>
          <div className="menu-item" onClick={() => setIsUserMenuOpen(false)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>Help</div>
          <div className="menu-separator" style={{height: '1px', background: 'var(--border-subtle)', margin: '4px 0'}} />
          <div className="menu-item" onClick={() => setIsUserMenuOpen(false)} style={{color: '#ff4d4d'}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Logout</div>
        </div>
      )}
